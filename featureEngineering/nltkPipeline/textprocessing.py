...
class TextProcessing:
    ...
    def run(self, file_name: str, version: int):
        ...
        PATH_DATA_RAW = os.path.join(repo_root, 'Data', 'data_raw')
        PATH_DATA_PROCESSED = os.path.join(repo_root, 'Data', 'data_processed')
        # reading JSON data
    ...
    def get_data_path(self, relative_path: str) -> str:
        """Get absolute path relative to the repository root."""
        script_dir = os.path.dirname(os.path.abspath(__file__))
        repo_root = os.path.join(script_dir, '..')
...
if __name__ == "__main__":
    ...
    PATH_DATA_PROCESSED = text_processing.get_data_path("Data/data_processed")

    text_processing.split(PATH_DATA_PROCESSED,"tickets_classification_eng_2.csv")

    #to run this fron the repo root execute